...
export default async function handler(req,res){
  ...
  let body=req.body
  if(typeof body==='string'){try{body=JSON.parse(body)}catch{return res.status(400).json({success:false,error:'Invalid JSON'})}}
  const scan=body?.scan
  if(!scan?.address||!scan?.intelligence) return res.status(400).json({success:false,error:'Scan data is required.'})

  const compact={token:scan.token,market:scan.market,trading:scan.trading,security:scan.security,intelligence:scan.intelligence,pair:scan.pair}
...
Do not tell the user to risk money they cannot afford to lose.`
  ...
  try{
    ...
    return res.status(200).json({success:true,model:'deterministic-fallback',analysis})
  }
}
