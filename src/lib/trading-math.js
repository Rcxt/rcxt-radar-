...
export function buildExecutionChecklist({scan,analytics,tape,positionSize=0}){
  ...
  const flowKnown=flowNet!==null&&flowBuyPct!==null
  ...
  const items=[
    ...
    },
    {
      ...
      detail:liquidity===null?'Liquidity unavailable':('
    },
    {
      key:'setup',
...
export function buildRugRiskChecklist({scan,tape}){
  ...
  if(!security.available){
    ...
    add(
      'evm-bytecode',
      'Contract bytecode',
      security.contractCodePresent===true?'pass':security.contractCodePresent===false?'critical':'unknown',
      security.contractCodePresent===true?'Contract bytecode is present.':security.contractCodePresent===false?'No contract bytecode was found.':'Bytecode status is unavailable.',
      'measured'
    )
    add(
      'evm-honeypot',
      'Honeypot simulation',
      evm.honeypot===true?'critical':evm.honeypot===false?'pass':'unknown',
      evm.honeypot===true?'GoPlus reports honeypot behavior.':evm.honeypot===false?'GoPlus did not detect honeypot behavior.':'Honeypot simulation is unavailable.',
      'measured'
    )
    add(
      'evm-source',
      'Contract source',
      evm.openSource===true?'pass':evm.openSource===false?'warning':'unknown',
      evm.openSource===true?'Contract source is available for inspection.':evm.openSource===false?'Contract source is not verified/open.':'Source verification is unavailable.',
      'measured'
    )
    const maxTax=Math.max(optionalNumber(evm.buyTaxPercent)||0,optionalNumber(evm.sellTaxPercent)||0)
    const taxKnown=evm.buyTaxPercent!=null||evm.sellTaxPercent!=null
    add(
      'evm-tax',
      'Buy / sell tax',
      !taxKnown?'unknown':maxTax>=50?'critical':maxTax>=10?'warning':'pass',
      taxKnown?`Buy ${optionalNumber(evm.buyTaxPercent)||0}% · Sell ${optionalNumber(evm.sellTaxPercent)||0}%`:'Token tax data is unavailable.',
      'measured'
    )
  }else{
    add(
  ...
  add(
    'seller-pressure',
    'Recent order flow',
    buy1==null?'unknown':buy1<30?'critical':buy1<42?'warning':buy1>85?'watch':'pass',
    buy1==null?'1h transaction flow is unavailable.':`1h transaction count is ${buy1.toFixed(1)}% buys.`,
    'measured'
  )
  ...
  add(
    ...
    'Price stretch',
    !priceKnown?'unknown':
      ((h1!==null&&h1>150)||(h24!==null&&h24>1000))?'warning':
      (h1!==null&&h1<-40)?'critical':
      ((h1!==null&&h1>60)||(h24!==null&&h24>400))?'watch':'pass',
    !priceKnown
      ? '1h/24h momentum data is unavailable.'
      : `Price change: 1h ${h1==null?'—':(h1>=0?'+':'')+h1.toFixed(1)+'%'} · 24h ${h24==null?'—':(h24>=0?'+':'')+h24.toFixed(1)+'%'}.`,
    'measured'
  )
...
+Math.round(flowNet).toLocaleString()+' net · '+flowBuyPct.toFixed(1)+'% buy volume'):'Waiting for usable trade tape',
    },
    {
      key:'chase',
      label:'Not excessively extended',
