...
export function analyzeCandles(candles, { intervalMinutes = 5 } = {}) {
  ...
  const supportLevels = localLevels(clean, 'support')
  const resistanceLevels = localLevels(clean, 'resistance')
  const nearestSupport = [...supportLevels].filter((value) => value < latestPrice).sort((a,b)=>b-a)[0] || supportLevels.at(-1) || null
  const nearestResistance = [...resistanceLevels].filter((value) => value > latestPrice).sort((a,b)=>a-b)[0] || resistanceLevels[0] || null
  const downsideToSupport = nearestSupport && latestPrice > 0 ? ((latestPrice - nearestSupport) / latestPrice) * 100 : null
  const upsideToResistance = nearestResistance && latestPrice > 0 ? ((nearestResistance - latestPrice) / latestPrice) * 100 : null
  const structureRiskReward = downsideToSupport && downsideToSupport > 0 && upsideToResistance != null
  ...
  const volatilityRegime =
    ...
    atrPercent >= 5 || volatility >= 5 ? 'ELEVATED' : 'CALMER'
  ...
  return {
    ...
    levels: {
      ...
      downsideToSupportPercent: round(downsideToSupport,1),
      upsideToResistancePercent: round(upsideToResistance,1),
      structureRiskReward: round(structureRiskReward,2),
    },
    regime: {
      volatility: volatilityRegime,
      structure:
        trend === 'BULLISH' && change1h > 0 ? 'UPTREND' :
        trend === 'BEARISH' && change1h < 0 ? 'DOWNTREND' :
        Math.abs(change1h) < 5 ? 'CHOP' : 'TRANSITION',
    },
    forecast: {
