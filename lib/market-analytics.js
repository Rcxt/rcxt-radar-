...
export function rsi(values, period = 14) {
  ...
  if (avgLoss <= EPSILON && avgGain <= EPSILON) return 50
...
function horizonChange(candles, intervalMinutes, horizonMinutes) {
  const bars = Math.ceil(horizonMinutes / Math.max(1, intervalMinutes))
  if (!Array.isArray(candles) || candles.length < bars + 1) return null
  const from = candles[candles.length - bars - 1]?.close
...
function rollingVwap(candles, period = 20) {
  const slice=(candles||[]).slice(-period)
  let numerator=0
  let denominator=0
  for(const candle of slice){
    const volume=Number(candle?.volume||0)
    const typical=(Number(candle?.high||0)+Number(candle?.low||0)+Number(candle?.close||0))/3
    if(volume>0&&typical>0){
...
function bollinger(values, period = 20, deviations = 2) {
  const nums=(values||[]).filter(Number.isFinite).slice(-period)
  if(nums.length<2) return {middle:null,upper:null,lower:null,widthPercent:null}
  const middle=mean(nums)
  const sigma=standardDeviation(nums)
  const upper=middle+sigma*deviations
  const lower=Math.max(0,middle-sigma*deviations)
  const widthPercent=middle>0?((upper-lower)/middle)*100:null
...
export function analyzeCandles(candles, { intervalMinutes = 5 } = {}) {
  ...
  const vwap20 = rollingVwap(clean,20)
  ...
  const change15m = horizonChange(clean, intervalMinutes, 15)
  const change1h = horizonChange(clean, intervalMinutes, 60)
  const change6h = horizonChange(clean, intervalMinutes, 360)
  ...
  if (Number.isFinite(change15m) && Number.isFinite(change1h) && change15m > 0 && change1h > 0) {
    bias += 9; reasons.push('15m and 1h momentum agree')
  }
  if (Number.isFinite(change15m) && Number.isFinite(change1h) && change15m < 0 && change1h < 0) {
    bias -= 10; risks.push('15m and 1h momentum are both negative')
  }
  if (Number.isFinite(change1h) && change1h < -25) { bias -= 18; risks.push('1h price structure is collapsing') }
  if (Number.isFinite(change15m) && change15m > 25) { bias -= 8; risks.push('Short-term move is extended') }
  ...
  if (vwap20 != null && latestPrice > vwap20) { bias += 5; reasons.push('Price is above recent volume-weighted value') }
  if (vwap20 != null && latestPrice < vwap20) { bias -= 5; risks.push('Price is below recent volume-weighted value') }
  if (bands.upper != null && latestPrice > bands.upper) risks.push('Price is above its recent volatility band')
  if (bands.lower != null && latestPrice < bands.lower) risks.push('Price is below its recent volatility band')
  ...
  const structure =
    ...
    Number.isFinite(change1h) && Math.abs(change1h) < 8 ? 'RANGE' : 'TRANSITION'

  const phase =
    bands.widthPercent != null && bands.widthPercent <= 10 ? 'COMPRESSION' :
    bands.widthPercent != null && bands.widthPercent >= 45 ? 'VOLATILITY_EXPANSION' :
    volumeAcceleration >= 1.4 && Number.isFinite(change1h) && Math.abs(change1h) >= 8 ? 'EXPANSION' :
  ...
  const modelConfidence = clamp(Math.round(
    ...
    (Number.isFinite(change15m) && Number.isFinite(change1h) && Math.sign(change15m) === Math.sign(change1h) ? 10 : 0)
  ), 15, 92)
  ...
  return {
    ...
    indicators:{
      ...
      vwapDistancePercent:vwap20&&latestPrice>0?round(((latestPrice/vwap20)-1)*100,1):null,
      bollingerMiddle:round(bands.middle,12),
      bollingerUpper:round(bands.upper,12),
      bollingerLower:round(bands.lower,12),
      bollingerWidthPercent:round(bands.widthPercent,1),
