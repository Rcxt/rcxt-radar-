...
function chainAliases(value) {
  const chain=String(value||'solana').trim().toLowerCase()
  return {
    solana:['solana','pumpfun','pump.fun'],
    robinhood:['robinhood chain','robinhood crypto'],
    ethereum:['ethereum','eth'],
    base:['base chain','base crypto'],
    bsc:['bnb chain','bsc'],
    arbitrum:['arbitrum'],
...
export function normalizeTerms(input = {}) {
  ...
  const chain = String(input.chain || 'solana').trim().toLowerCase()
  return { address, symbol, name, chain }
}
...
export function buildXQuery(input = {}) {
  const { address, symbol, name, chain } = normalizeTerms(input)
  const parts = []
  ...
  if (symbol.length >= 4) {
    ...
    const chainContext=chainAliases(chain)
      .map((term)=>term.includes(' ')?'"'+cleanQueryTerm(term)+'"':cleanQueryTerm(term))
      .join(' OR ')
    parts.push('(' + cleanSymbol + ' (' + chainContext + ' OR memecoin OR crypto OR token))')
  }
...
function postRelevance(text, input = {}) {
  ...
  if (symbol) {
    ...
    const genericCryptoContext=/(memecoin|meme coin|crypto|token|market cap|\bmc\b|dex|ca:|contract)/i.test(body)
    const chainContext=chainAliases(chain).some((term)=>lower.includes(term.toLowerCase()))
    const hasCryptoContext=genericCryptoContext||chainContext
