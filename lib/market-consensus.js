...
function spreadPercent(values) {
  ...
  const middle = median(rows)
  if (!middle) return null
  return (Math.max(...rows) - Math.min(...rows)) / middle * 100
}
...
async function fetchGeckoTerminal(address, chain = getChain('solana')) {
  try {
    const data = await fetchJson(
      `https://api.geckoterminal.com/api/v2/networks/${encodeURIComponent(chain?.geckoterminal || 'solana')}/tokens/${encodeURIComponent(address)}`,
      {
        headers:{
...
export function buildMarketConsensus(pair, providers = {}) {
  ...
  const maxDeviationPercent = priceRows.length
    ...
    : null
  const priceSpreadPercent = spreadPercent(priceRows.map((row) => row.priceUsd))

  const priceProviderCount = priceRows.length
  const externalPriceProviderCount = Math.max(0, priceProviderCount - (primaryPrice ? 1 : 0))
  // Pairwise spread is safer than median deviation for two-source comparisons:
  // two prices can be 20%+ apart while each sits <12% from their midpoint.
  const priceConflict = priceProviderCount >= 2 && Number(priceSpreadPercent || 0) >= 12
  const priceAgreement = priceProviderCount >= 3 && Number(priceSpreadPercent || 0) <= 5

  const liquidityRows = [
    finiteNumber(pair?.liquidity?.usd) !== null && Number(pair?.liquidity?.usd) > 0
      ? { source:'dexscreener', scope:'selected-pair', liquidityUsd:Number(pair.liquidity.usd) }
      : null,
    providers?.geckoterminal?.available && finiteNumber(providers.geckoterminal.liquidityUsd)
      ? { source:'geckoterminal', scope:'token-total', liquidityUsd:Number(providers.geckoterminal.liquidityUsd) }
      : null,
    providers?.birdeye?.available && finiteNumber(providers.birdeye.liquidityUsd)
      ? { source:'birdeye', scope:'token-overview', liquidityUsd:Number(providers.birdeye.liquidityUsd) }
      : null,
  ].filter(Boolean)

  // Liquidity numbers from different scopes are not directly comparable.
  // DexScreener is the selected pair, while GeckoTerminal token reserve is
  // token-wide across pools. Unknown provider scopes remain visible but never
  // create a false conflict penalty.
  const liquidityGroups = new Map()
  for (const row of liquidityRows) {
    const current=liquidityGroups.get(row.scope)||[]
    current.push(row)
    liquidityGroups.set(row.scope,current)
  }
  let liquidityConflict = false
  let liquidityComparableProviderCount = 0
  for (const rows of liquidityGroups.values()) {
    liquidityComparableProviderCount=Math.max(liquidityComparableProviderCount,rows.length)
    if(rows.length<2) continue
    const values=rows.map((row)=>row.liquidityUsd).filter((value)=>value>0)
    if(values.length<2) continue
    const low=Math.min(...values)
    const high=Math.max(...values)
    if(low>0&&high/low>=3) liquidityConflict=true
  }

  const marketCapRows = [
    finiteNumber(pair?.marketCap) !== null && Number(pair?.marketCap) > 0
      ? { source:'dexscreener', marketCapUsd:Number(pair.marketCap), primary:true }
      : null,
    providers?.geckoterminal?.available && finiteNumber(providers.geckoterminal.marketCapUsd) !== null && Number(providers.geckoterminal.marketCapUsd) > 0
      ? { source:'geckoterminal', marketCapUsd:Number(providers.geckoterminal.marketCapUsd) }
      : null,
    providers?.birdeye?.available && finiteNumber(providers.birdeye.marketCapUsd) !== null && Number(providers.birdeye.marketCapUsd) > 0
      ? { source:'birdeye', marketCapUsd:Number(providers.birdeye.marketCapUsd) }
      : null,
  ].filter(Boolean)

  const medianMarketCapUsd = median(marketCapRows.map((row) => row.marketCapUsd))
  const marketCapSpreadPercent = spreadPercent(marketCapRows.map((row) => row.marketCapUsd))
  const marketCapProviderCount = marketCapRows.length
  // Market-cap providers can legitimately differ on circulating-supply methods,
  // so use a wider threshold than live price before calling it a conflict.
  const marketCapConflict = marketCapProviderCount >= 2 && Number(marketCapSpreadPercent || 0) >= 35
  ...
  if (liquidityComparableProviderCount >= 2 && !liquidityConflict) evidenceScore += 8
  if (liquidityConflict) evidenceScore -= 10
  if (marketCapProviderCount >= 2 && !marketCapConflict) evidenceScore += 5
  if (marketCapConflict) evidenceScore -= 10
  ...
  return {
    ...
    priceConflict,
    liquidityConflict,
    liquidityComparableProviderCount,
    marketCapProviderCount,
    medianMarketCapUsd,
    marketCapSpreadPercent,
    marketCapConflict,
    evidenceScore,
    prices:priceRows,
    auxiliaryPrices:[
...
export async function getMarketConsensus(address, pair = null, chain = 'solana') {
  const config = typeof chain === 'string' ? getChain(chain) : chain
  const cacheKey = `${config?.id || 'solana'}:${String(address || '').toLowerCase()}`
  const cached = marketCache.get(cacheKey)
  if (cached && Date.now() - cached.time < cached.ttlMs) {
    return buildMarketConsensus(pair, cached.providers)
  }

  const solana = config?.family !== 'evm'
  const [geckoterminal, jupiterPrice, helius, birdeye] = await Promise.all([
    fetchGeckoTerminal(address,config),
    solana ? fetchJupiterPrice(address) : Promise.resolve({ available:false, source:'jupiter-price', reason:'SOLANA_ONLY' }),
    solana ? fetchHeliusAsset(address) : Promise.resolve({ available:false, source:'helius', reason:'SOLANA_ONLY' }),
    solana ? fetchBirdeyeOverview(address) : Promise.resolve({ available:false, source:'birdeye-market', reason:'SOLANA_ONLY' }),
  ])
