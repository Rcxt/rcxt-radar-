...
async function dexFetch(path, revalidate = 10) {
...
export async function getBestPair(address) {
  try {
    ...
    return pairs.reduce((best, pair) => {
      if (!best) return pair
      return Number(pair?.liquidity?.usd || 0) > Number(best?.liquidity?.usd || 0)
        ? pair
        : best
    }, null)
  } catch {
...
export async function getPairsForTokens(addresses) {
  ...
  for (let i = 0; i < unique.length; i += 30) {
    ...
    try {
      const pairs = await dexFetch(`/tokens/v1/solana/${chunk.join(',')}`, 15)
      ...
      // Keep partial wallet enrichment if one batch fails.
    }
  }
...
export async function getRadarCandidates(limit = 8) {
  try {
    const boosts = await dexFetch('/token-boosts/top/v1', 20)
