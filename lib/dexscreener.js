...
export async function getBestPair(address, chain = 'solana') {
  ...
  const cached = readCached(pairCache, cacheKey, 3500, 60_000)
  if (cached.fresh) return cached.fresh

  try {
    const pairs = await dexFetch(
      `/tokens/v1/${encodeURIComponent(config.dexscreener)}/${encodeURIComponent(address)}`,
      { freshMs:3500, staleMs:60_000 },
    )
    ...
    const best = candidates.reduce((current, pair) => {
...
export async function resolveDexChain(address) {
  if (!looksLikeEvmAddress(address)) return getChain('solana')
  ...
  try {
    ...
    const candidates = pairs
      .map((pair) => ({ pair, chain:getChain(pair?.chainId) }))
      .filter(({ pair, chain }) => {
        if (!chain || chain.family !== 'evm') return false
        return String(pair?.baseToken?.address || '').toLowerCase() === normalized
      })
