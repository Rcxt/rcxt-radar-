...
export const SCORE_VERSION = '4.0.0'
...
export function analyzePair(pair, mintSecurity = null) {
  ...
  if (mintSecurity?.concentrationAvailable) {
    if (top1 !== null && top1 >= 70) {
      safetyScore -= 30; breakdown.concentration -= 12; negatives.push('Largest token account controls an extreme share of supply'); riskFlags.push('EXTREME_ACCOUNT_CONCENTRATION')
    } else if (top1 !== null && top1 >= 45) {
      safetyScore -= 16; breakdown.concentration -= 6; negatives.push('Largest token account concentration is elevated'); riskFlags.push('HIGH_ACCOUNT_CONCENTRATION')
    } else if (top1 !== null && top1 <= 20) {
      safetyScore += 5; breakdown.concentration += 2
    }

    if (top10 !== null && top10 >= 92) {
      safetyScore -= 12; breakdown.concentration -= 5; negatives.push('Top token accounts hold most of the supply'); riskFlags.push('TOP10_CONCENTRATION')
    } else if (top10 !== null && top10 <= 65) {
      safetyScore += 5; breakdown.concentration += 2; positives.push('Token-account concentration is comparatively distributed')
    }
  }
  ...
  if (mintSecurity?.concentrationAvailable) dataQualityScore += 7
  ...
  let signal = 'WATCH'
  if (contractDanger || concentrationDanger || safetyScore < 30 || executionScore < 25) {
    signal = 'SELL / AVOID'
  } else if (collapseDanger || sellerDanger || setupScore < 32 || (score < 47 && (setupScore < 45 || executionScore < 45))) {
    signal = 'REDUCE'
  } else {
  ...
  if (parabolic) marketState = 'EXTENDED'
  ...
  return {
    ...
    concentration: { available:Boolean(mintSecurity?.concentrationAvailable), top1Percent:top1, top5Percent:top5, top10Percent:top10 },
    scoring: { setupWeight:45, executionWeight:30, safetyWeight:25, meaning:'Risk-adjusted setup score, not a probability of profit' },
