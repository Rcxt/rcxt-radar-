...
export function analyzePair(pair, mintSecurity = null) {
  ...
  const ageHours = pair?.pairCreatedAt
    ...
    : null
  const liquidityRatio = liquidityReported && reportedMarketCap > 0 ? liquidity / reportedMarketCap : null
  ...
  if (reportedMarketCap > 0 && liquidityReported) {
  ...
  const securityProviderNames = Array.isArray(externalEvidence?.providers) ? externalEvidence.providers : []
  const externalOnlySecurityProviderCount = securityProviderNames.filter(
    (provider) => !['solana-rpc','evm-rpc'].includes(String(provider || '').toLowerCase())
  ).length
  ...
  const tradeQuality = marketEvidence?.tradeQuality || {}
  const tradeQualityAvailable = tradeQuality?.available === true
  const tradeManipulationRisk = tradeQualityAvailable && finite(tradeQuality?.manipulationRiskScore)
    ? Number(tradeQuality.manipulationRiskScore)
    : null
  const tradeSampleSize = tradeQualityAvailable ? n(tradeQuality?.summary?.sampleSize) : 0
  ...
  const evmCannotBuy = goPlusEvidence?.cannotBuy === true
  ...
  if (evmSecurity) {
    ...
    if (goPlusEvidence?.available) {
      ...
      if (evmCannotBuy) {
        const observedRecentBuys =
          (flow1Reported && buys1 > 0) ||
          (flow24Reported && buys24 > 0)

        if (observedRecentBuys) {
          safetyScore -= 4
          executionScore = clamp(executionScore - 8, 0, 100)
          negatives.push('GoPlus buy simulation conflicts with observed DEX buys; treat contract simulation as uncertain')
          riskFlags.push('EVM_BUY_SIMULATION_CONFLICT')
        } else {
          safetyScore -= 8
          executionScore = clamp(executionScore - 35, 0, 100)
          negatives.push('GoPlus simulation reports that buying may be blocked')
          riskFlags.push('EVM_CANNOT_BUY')
        }
      }
  ...
  if (marketPriceConflict) {
    ...
    positives.push('Price is corroborated across multiple market data sources')
  }
  ...
  if (tradeManipulationRisk !== null) {
    if (tradeManipulationRisk >= 70) {
      setupScore = clamp(setupScore - 14, 0, 100)
      executionScore = clamp(executionScore - 12, 0, 100)
      negatives.push('Recent trade tape is heavily concentrated or churn-like')
      riskFlags.push('ACTIVITY_QUALITY_HIGH_RISK')
    } else if (tradeManipulationRisk >= 45) {
      setupScore = clamp(setupScore - 8, 0, 100)
      executionScore = clamp(executionScore - 6, 0, 100)
      negatives.push('Recent trade tape shows concentrated or repetitive activity')
      riskFlags.push('ACTIVITY_QUALITY_RISK')
    } else if (tradeManipulationRisk >= 25) {
      setupScore = clamp(setupScore - 3, 0, 100)
      executionScore = clamp(executionScore - 3, 0, 100)
      riskFlags.push('ACTIVITY_QUALITY_CAUTION')
    } else if (tradeSampleSize >= 30) {
      positives.push('Recent trade sample does not show strong churn/concentration flags')
    }
  }
  ...
  // "Currently dangerous" is intentionally narrower than "has mutable controls".
  // A transfer hook or live pause authority must block full verification/promotion,
  // but it is not equivalent to a token that is already paused/frozen/unusable.
  ...
  if (mintSecurity?.available) dataQualityScore += 8
  if (concentrationAvailable) dataQualityScore += 7
  if (externalOnlySecurityProviderCount >= 2) dataQualityScore += 8
  else if (externalOnlySecurityProviderCount === 1) dataQualityScore += 5
  ...
  if (tradeManipulationRisk !== null && tradeManipulationRisk >= 70) dataQualityScore -= 8
  else if (tradeManipulationRisk !== null && tradeManipulationRisk >= 45) dataQualityScore -= 4
  else if (tradeManipulationRisk !== null && tradeManipulationRisk < 20 && tradeSampleSize >= 30) dataQualityScore += 3
  ...
  applyScoreCap('EVM_CANNOT_BUY', 45, riskFlags.includes('EVM_CANNOT_BUY'))
  applyScoreCap('EVM_BUY_SIMULATION_CONFLICT', 65, riskFlags.includes('EVM_BUY_SIMULATION_CONFLICT'))
  ...
  applyScoreCap('ACTIVITY_QUALITY_HIGH_RISK', 58, tradeManipulationRisk !== null && tradeManipulationRisk >= 70)
  applyScoreCap('ACTIVITY_QUALITY_RISK', 68, tradeManipulationRisk !== null && tradeManipulationRisk >= 45)
  ...
  const transactionDepthConfidence = flow24Reported
    ? Math.min(10, Math.log10(Math.max(1,tx24)) * 3)
    : 0
  let confidence = clamp(
    Math.round(dataQualityScore * 0.88 + transactionDepthConfidence),
    10, 96
  )
  ...
  if (riskFlags.includes('EVM_BUY_SIMULATION_CONFLICT')) confidence = Math.min(confidence, 60)
  ...
  if (tradeManipulationRisk !== null && tradeManipulationRisk >= 70) confidence = Math.min(confidence, 55)
  else if (tradeManipulationRisk !== null && tradeManipulationRisk >= 45) confidence = Math.min(confidence, 65)
  if (marketPriceAgreement && externalPriceProviderCount >= 2) confidence = Math.min(98, confidence + 3)
  // Confidence is heuristic evidence quality, not a calibrated probability.
  // Round down to 5-point bands to avoid false precision.
  confidence = clamp(Math.floor(confidence / 5) * 5, 10, 95)
  ...
  const leanBuyMissing = [
    ...
    evmCannotBuy ? 'EVM buy simulation is not cleanly verified' : null,
    tradeManipulationRisk !== null && tradeManipulationRisk >= 45 ? 'Recent trade activity is too concentrated or churn-like' : null,
  ...
  return {
    ...
    score,
    confidenceType:'EVIDENCE_CONFIDENCE_NOT_PROFIT_PROBABILITY',
    scoreBeforeCaps,
    scoreCaps,
    setupScore,
    executionScore,
    ...
    buyPercent24h: flow24Reported ? round(buyPct24) : null,
    buyPercent1h: flow1Reported ? round(buyPct1) : null,
    buyPercent5m: flow5Reported ? round(buyPct5) : null,
    dataAvailability:{
      price:priceReported,
      marketCap:reportedMarketCap > 0,
      fdv:fdv > 0,
      liquidity:liquidityReported,
      volume:{m5:volume5Reported,h1:volume1Reported,h6:volume6Reported,h24:volume24Reported},
      momentum:{m5:change5Reported,h1:change1Reported,h6:change6Reported,h24:change24Reported},
      flow:{m5:flow5Reported,h1:flow1Reported,h24:flow24Reported},
      age:ageHours !== null,
    },
    liquidityToCapPercent: liquidityReported && reportedMarketCap > 0 ? round(liquidityRatio * 100, 2) : null,
    valuationBasis,
    liquidityReported,
    ...
    securityEvidence: {
      ...
      market:{
        ...
        marketCapConflict,
        tradeQuality:tradeQualityAvailable ? {
          sampleQuality:tradeQuality?.sampleQuality || null,
          sampleSize:tradeSampleSize,
          manipulationRiskScore:tradeManipulationRisk,
          activityQualityScore:finite(tradeQuality?.activityQualityScore) ? Number(tradeQuality.activityQualityScore) : null,
          flags:Array.isArray(tradeQuality?.flags) ? tradeQuality.flags : [],
          summary:tradeQuality?.summary || null,
        } : null,
      },
    },
