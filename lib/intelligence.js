...
export const SCORE_VERSION = '6.1.0-beta.1'
...
export function analyzePair(pair, mintSecurity = null) {
  ...
  const providerStructuralDanger =
    ...
    externalHardFlags.includes('GOPLUS_BLACKLIST') ||
    externalHardFlags.includes('GOPLUS_DEFAULT_FROZEN') ||
    externalHardFlags.includes('GOPLUS_TRANSFER_PAUSABLE') ||
    externalHardFlags.includes('GOPLUS_HONEYPOT') ||
    externalHardFlags.includes('GOPLUS_CANNOT_SELL') ||
  ...
  if (evmSecurity) {
    ...
    if (goPlusEvidence?.available) {
      if (evmOpenSource === false) {
        ...
        safetyScore += 10
        breakdown.contract += 4
        positives.push('EVM contract is open source and no hard GoPlus contract veto is present')
      }
  ...
  const concentrationAvailable = ownerConcentrationAvailable || accountConcentrationAvailable || externalConcentrationAvailable
  const concentrationMethod = ownerConcentrationAvailable ? 'RESOLVED_TOKEN_ACCOUNT_OWNERS' :
    accountConcentrationAvailable ? 'TOKEN_ACCOUNTS' :
    externalConcentrationAvailable ? (mintSecurity?.externalConcentrationSource || 'EXTERNAL_TOP_HOLDERS') : 'UNAVAILABLE'
  ...
  const top1 = ownerConcentrationAvailable ? ownerTop1 : accountConcentrationAvailable ? accountTop1 : externalTop1
  const top5 = ownerConcentrationAvailable ? ownerTop5 : accountConcentrationAvailable ? accountTop5 : externalTop5
  const top10 = ownerConcentrationAvailable ? ownerTop10 : accountConcentrationAvailable ? accountTop10 : externalTop10
  ...
  if (marketCap > 0) dataQualityScore += 6
  ...
  return {
    ...
    securityEvidence: {
      ...
      evm:evmSecurity ? {
        ...
        transferPausable:goPlusEvidence?.transferPausable === true,
      } : null,
      token2022:{
        detected:token2022,
        transferFeeEnabled,
      ...
      market:{
        ...
        liquidityConflict:marketEvidence?.liquidityConflict === true,
      },
    },
