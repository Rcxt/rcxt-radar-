...
export function analyzePair(pair, mintSecurity = null) {
  ...
  let signal = 'WATCH'
  if (score >= 82 && !riskFlags.includes('MINT_AUTHORITY_ACTIVE') && !riskFlags.includes('FREEZE_AUTHORITY_ACTIVE')) {
    signal = 'BUY SETUP'
  } else if (score >= 70) {
    signal = 'LEAN BUY'
  } else if (score >= 55) {
  ...
  if (score < 58 || riskFlags.length >= 2) risk = 'HIGH'
  if (score < 40 || riskFlags.length >= 4) risk = 'EXTREME'
