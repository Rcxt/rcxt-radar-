...
function holderConcentration(rows = []) {
  const normalized = rows
    .map((row) => {
      const percent = normalizePercent(row?.percent ?? row?.pct)
      const tag = compactText(row?.tag ?? row?.label ?? '', 80)
      const locked = activeStatus(row?.is_locked ?? row?.locked) === true
      const clearlyNonHolder = /burn|black\s*hole|dead\s*address|zero\s*address|liquidity|\blp\b|pool|pair|dex/i.test(tag)
      return {
        percent,
        tag:tag || null,
        locked,
        isContract:activeStatus(row?.is_contract) === true,
        excludedFromHolderRisk:locked || clearlyNonHolder,
      }
    })
    .filter((row) => row.percent !== null)
    .sort((a,b) => Number(b.percent) - Number(a.percent))

  const riskRows = normalized.filter((row) => !row.excludedFromHolderRisk)
  const sum = (items,count) => items.slice(0,count).reduce((total,row)=>total+Number(row.percent||0),0)
  const excludedPercent = normalized
    .filter((row)=>row.excludedFromHolderRisk)
    .reduce((total,row)=>total+Number(row.percent||0),0)

  return {
    rows:normalized.slice(0,10),
    rawTop1Percent:normalized.length ? Number(sum(normalized,1).toFixed(2)) : null,
    rawTop5Percent:normalized.length ? Number(sum(normalized,5).toFixed(2)) : null,
    rawTop10Percent:normalized.length ? Number(sum(normalized,10).toFixed(2)) : null,
    top1Percent:riskRows.length ? Number(sum(riskRows,1).toFixed(2)) : null,
    top5Percent:riskRows.length ? Number(sum(riskRows,5).toFixed(2)) : null,
    top10Percent:riskRows.length ? Number(sum(riskRows,10).toFixed(2)) : null,
    excludedPercent:Number(excludedPercent.toFixed(2)),
    excludedCount:normalized.filter((row)=>row.excludedFromHolderRisk).length,
  }
}
...
export function normalizeGoPlusSolana(data, address) {
  ...
  const holderStats = holderConcentration(holders)
  ...
  return {
    ...
    top1Percent:holderStats.top1Percent,
    top5Percent:holderStats.top5Percent,
    top10Percent:holderStats.top10Percent,
    rawTop1Percent:holderStats.rawTop1Percent,
    rawTop5Percent:holderStats.rawTop5Percent,
    rawTop10Percent:holderStats.rawTop10Percent,
    excludedHolderPercent:holderStats.excludedPercent,
    excludedHolderCount:holderStats.excludedCount,
    holderRows:holderStats.rows,
...
export function normalizeGoPlusEvm(data, address) {
  ...
  const slippageModifiable = activeStatus(token.slippage_modifiable)
  const personalSlippageModifiable = activeStatus(token.personal_slippage_modifiable)
  const antiWhale = activeStatus(token.is_anti_whale)
  const antiWhaleModifiable = activeStatus(token.anti_whale_modifiable)
...
export function mergeSecurityEvidence(onchain = {}, externalSecurity = {}, marketEvidence = null) {
  ...
  const warningRiskCount =
    ...
    (goplus?.takeBackOwnership ? 1 : 0) +
    (goplus?.mintable ? 1 : 0) +
    (goplus?.slippageModifiable ? 1 : 0) +
    (goplus?.personalSlippageModifiable ? 1 : 0) +
    (goplus?.antiWhaleModifiable ? 1 : 0) +
  ...
  if (goplus?.cannotBuy) hardRiskFlags.push('GOPLUS_CANNOT_BUY')
  ...
  if (goplus?.gasAbuse) hardRiskFlags.push('GOPLUS_GAS_ABUSE')
  ...
  if (goplus?.blacklistActive) softRiskFlags.push('GOPLUS_BLACKLIST_CAPABILITY')
  if (goplus?.transferPausable) softRiskFlags.push('GOPLUS_TRANSFER_PAUSABLE')
  if (goplus?.cannotSellAll) softRiskFlags.push('GOPLUS_CANNOT_SELL_ALL')
  if (goplus?.externalCall) softRiskFlags.push('GOPLUS_EXTERNAL_CALL')
  if (goplus?.slippageModifiable) softRiskFlags.push('GOPLUS_MODIFIABLE_TAX')
  if (goplus?.personalSlippageModifiable) softRiskFlags.push('GOPLUS_PERSONAL_TAX_CONTROL')
  if (goplus?.antiWhale) softRiskFlags.push('GOPLUS_ANTI_WHALE')
  if (goplus?.antiWhaleModifiable) softRiskFlags.push('GOPLUS_MODIFIABLE_ANTI_WHALE')
